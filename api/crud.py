...
def get_events(
    ...
    return query.order_by(Event.date.asc().nullslast(), Event.created_at.desc()).offset(skip).limit(limit).all()


def get_event(db: Session, event_id: int, owner_id: int | None = None):
